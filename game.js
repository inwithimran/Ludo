	...
	dialog: {
		...
		createButtons: function(a, b, c, d, e, g) {
			...
			if(b == a.dialog.type.ok || b == a.dialog.type.about){
			 h.x = d.world.centerX, f.x = d.world.centerX, f.bringToTop();
			}
			else{ 
...
Menu.prototype = {
	create: function() {
		...
		for (f = 0; f < 2; f++){ 
			...
			if(f==0){
				...
				k.width = 130;
				k.height = 65;
				// k = this.game.add.image(g.x, g.y - c , "icons_bottom", f); //this two line is in comment
				// k.anchor.setTo(.5);  //this one also
				a.add(g);a.add(k);
			}
		...
		k = new Button(this.game, this.game.world.centerX +100, h + m / 2 + 60, "atlas", 2 < main.storage.data.playerTypesCount[main.player.type.off] && main.storage.data.playerTypesMenu[0] == main.player.type.off ? "start2.png" : "start.png", this.btnStartPressed);
		...
		// main.menu.panelGroups[1].add(e);  //this line is under comment
		this.startBtn = k;
	...
	btnTypePressed: function(a) {
		...
		// if (a.properties.color == 0 && b[d] == 2)
		// {
		// 	main.menu.self.startBtn.frameName = 2 < c[main.player.type.off] ? "start2.png" : "start.png";
		// }
		// else
		// {
		main.menu.self.startBtn.frameName = (2 < c[main.player.type.off]) || (main.storage.data.playerTypesMenu[0]== 2) ? "start2.png" : "start.png";
		// }
		// main.menu.self.startBtn.frameName = 2 < c[main.player.type.off] && !main.storage.data.gameSaved ? "start2.png" : "start.png";
		
		// console.log("A properties COLOR :: " + a.properties.color);
		// console.log("A properties COLOR :: " + b[d]);
		main.managers.storage.save();
		main.managers.audio.playSound("menu_click")
	},
	...
	btnStartPressed: function() {
		...
		if (main.storage.data.playerTypesMenu[0] == 2)
		{
			main.dialog.show(main, main.dialog.type.ok, main.managers.language.get("first_player"));
            main.managers.audio.playSound("menu_click");
		}
		else
		{
...
Board.prototype = {
	...
	setup: function(a) {
		for (var b = this.game.add.graphics(0, 0), c, d, e, g, h = 0; h < main.game.maxPlayers; h++) c = main.board.x + main.board.BG_YS[h] * main.board.squareSize + main.board.BG_OFFSETS[h], d = main.board.y + main.board.BG_XS[h] *
			main.board.squareSize + main.board.BG_OFFSETS[4 + h], e = main.board.BG_WS[h] * main.board.squareSize + main.board.BG_OFFSETS[8 + h], g = main.board.BG_HS[h] * main.board.squareSize + main.board.BG_OFFSETS[12 + h], this.game.add.image(main.board.HOME_XS[h], main.board.HOME_YS[h], "atlas", "home.png"), ++a == main.game.maxPlayers && (a = 0);
...
var Token = function(a, b, c, d) {
	...
	this.countText = a.add.text(0, 2, "", {
		font: main.fonts.tokenFontSize,
		fill: 1 == c.color ? "#000000" : "#000000",
		align: "center"
	});
